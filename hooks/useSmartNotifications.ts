...
// VHI cache TTL: matches the vhiCycle server cadence (every 15 min)
const VHI_CACHE_TTL_MS = 15 * 60 * 1000;
...
export const useSmartNotifications = (
  ...
  // Cache VHI data so we don't re-fetch on every notification check cycle
  const cachedVhiRef = useRef<VHI | null>(null);
  const vhiLastFetchedRef = useRef<number>(0);
  ...
  const checkAndScheduleNotifications = useCallback(async () => {
    ...
    try {
      ...
      // Fetch VHI with a 15-min TTL so we don't hit the API on every check
      if (now - vhiLastFetchedRef.current > VHI_CACHE_TTL_MS) {
        try {
