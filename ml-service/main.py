...
from api.endpoints import router, set_papagei_model as _set_endpoint_model
from api.snp_parser import router as snp_router
from api.note_parser import router as note_router
from api.forecast_composite import router as forecast_router
from api.twin_forecast import router as twin_forecast_router
from api.medgemma_explain import router as explain_router
try:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        _set_endpoint_model(papagei_model)  # expose to /api/ppg/embeddings router
...
app = FastAPI(
    title="Nuralix ML Service",
    description=(
        "ML backend: PPG analysis · SNP/genomics parsing · clinical note parsing · "
        "composite risk forecasting · vital time-series forecasting (Chronos/TimesFM/Granite) · "
        "clinical & genomic explanations (MedGemma/BioGPT) · causal risk calibration (DoWhy/EconML)"
    ),
    version="2.0.0",
    lifespan=lifespan,
)
...
app.include_router(twin_forecast_router)
...
async def health_check():
    """Health check endpoint — reports status of all ML subsystems."""
    from api.twin_forecast import _chronos_pipeline, _timesfm_model, _granite_pipeline
    from api.medgemma_explain import _medgemma_api, _medgemma_local, _biogpt_pipeline
    from api.genomics_models import ENABLED as GENOMICS_ENABLED, status as genomics_status

    return {
        "status": "healthy",
        # PPG / PaPaGei — drives /api/ppg/analyze and /api/ppg/embeddings
        "papagei_loaded": papagei_model is not None,
        "ppg_embeddings_available": papagei_model is not None,
        # Vital forecasting models
        "twin_forecast": {
            "chronos_loaded": _chronos_pipeline is not None,
            "timesfm_loaded": _timesfm_model is not None,
            "granite_loaded": _granite_pipeline is not None,
            "scipy_available": True,
        },
        # Clinical / genomic explanation models
        "explain": {
            "medgemma_api": _medgemma_api is not None,
            "medgemma_local": _medgemma_local is not None,
            "biogpt": _biogpt_pipeline is not None,
            "openai_fallback": bool(os.getenv("OPENAI_API_KEY")),
        },
        # Genomic transformer models (opt-in via GENOMICS_MODELS_ENABLED=1)
        "genomics": {"enabled": GENOMICS_ENABLED, **genomics_status()},
        # Always-available services
        "snp_parser": True,
        "note_parser": True,
        "forecast_composite": True,
    }
