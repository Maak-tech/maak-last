# Nora Migration Guide

## Overview

The Nora AI service has been refactored to be **HIPAA-safe by design**. This guide explains how to migrate from the old API to the new one.
...
import { analyze } from './services/nora/analyze';
...
```typescript
// New: index.ts exports runNoraAnalysis()
...
Use the new `runNoraAnalysis()` for full control:
...
async function handleFallDetection(
  ...
  // Nora will automatically escalate falls
  if (result.success && result.output) {
    const actions = await executeNoraActions(
      result.output,
      alertContext,
...
| N/A | Backend actions via `executeNoraActions()` | New deterministic mapping |
...
- OR use `executeNoraActions()` for automatic mapping
...
**New:** Always succeeds
```typescript
const result = await runNoraAnalysis(request);
...
import { analyze } from './services/nora';
// or
import { analyze } from './services/nora/adapter';
```
...
const result = await runNoraAnalysis(request);
const actions = await executeNoraActions(result.output, context, traceId);
