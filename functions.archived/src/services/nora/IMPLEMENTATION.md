# Nora AI Implementation Summary
...
├── index.ts              ✅ Public API with runNoraAnalysis()
...
├── observability.ts      ✅ Metrics (nora.calls, failures, guardrail_blocks)
...
**Data Flow:**
```
AlertContext (PHI) → inputBuilder → NoraInput (NO PHI) → LLM → RawAIResponse → NoraOutput (NO PHI)
```
...
**Result:** `runNoraAnalysis()` ALWAYS returns `success: true`
...
NORA_ENABLED=true                # Enable/disable (default: true)
NORA_MODEL=gpt-4o-mini           # Model name
NORA_TIMEOUT_MS=8000             # Timeout
NORA_MAX_RETRIES=2               # Retry attempts
```
...
import { handleVitalAlertWithNora } from '../services/nora/example-usage';

// Replace existing alert logic
await handleVitalAlertWithNora(alertId, patientId, vitalType, value, severity);
```

### 2. Symptom Alerts
```typescript
// Similar pattern for symptom alerts
await runNoraAnalysis({
  traceId,
  alertContext: {
    alertType: 'symptom',
    severity: 'warning',
...
- ✅ **Access Controls:** Only authorized backend functions call Nora
...
Track these to measure Nora effectiveness:
...
2. **High Error Rate:** Disable AI via `NORA_ENABLED=false`
