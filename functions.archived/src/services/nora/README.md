# Nora AI Analysis Service

**HIPAA-safe AI orchestration for health alert analysis**

## Overview

Nora is a HIPAA-compliant AI service that analyzes health alerts and provides structured, actionable recommendations. It is designed with **PHI safety** as a core principle and implements a **fail-closed** architecture to ensure system reliability.
...
- `NoraInput` - Sanitized input for AI (NO PHI)
- `NoraOutput` - Structured output from AI (NO PHI)
...
**Principle:** Nora NEVER blocks critical health alerts
...
NoraOutput is mapped to **deterministic backend actions**:
...
```typescript
import { runNoraAnalysis } from './services/nora';
...
- `NORA_ENABLED` - Enable/disable AI (default: true)
- `NORA_LLM_PROVIDER` - LLM provider (default: 'openai')
- `NORA_MODEL` - Model name (default: 'gpt-4o-mini')
- `NORA_TIMEOUT_MS` - LLM timeout (default: 8000)
- `NORA_MAX_RETRIES` - Max retry attempts (default: 2)
...
- `nora.duration.*` - Duration buckets
...
Nora provides **informational risk assessment** only. It does NOT:
