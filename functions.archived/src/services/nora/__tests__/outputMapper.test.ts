/**
 * Unit tests for Nora output mapper
...
describe("Nora Output Mapper", () => {
  describe("mapToBackendActions", () => {
    ...
    describe("Automated actions mapping", () => {
      ...
      it("should always include log_nora_analysis", () => {
        const output = createTestOutput(
          RecommendedActionCode.MONITOR,
          EscalationLevel.NONE
        );
        const actions = mapToBackendActions(output, "trace_123");
        expect(actions.autoActions).toContain("log_nora_analysis");
      });
    });
  });

  describe("formatForAudit", () => {
    it("should format NoraOutput for audit log", () => {
      const output: NoraOutput = {
        riskScore: 65,
        summary: "Test summary",
