...
export function healthCheck(apiKey?: string): HealthStatus {
  ...
  try {
    ...
    const key =
      apiKey || process.env.NORA_API_KEY || process.env.OPENAI_API_KEY;
    if (!key) {
      warnings.push(
  ...
  logger.info("Health check completed", {
    ...
    fn: "nora.monitoring.healthCheck",
  });
...
export function getServiceStats(): ServiceStats {
  ...
  const totalCalls = metrics["nora.calls"] || 0;
  const failures = metrics["nora.failures"] || 0;
  const guardrailBlocks = metrics["nora.guardrail_blocks"] || 0;
  const aiCalls = metrics["nora.analysis_type.ai"] || 0;
  const deterministicCalls = metrics["nora.analysis_type.deterministic"] || 0;
...
export async function generateMonitoringReport(): Promise<MonitoringReport> {
  ...
  logger.info("Monitoring report generated", {
    ...
    fn: "nora.monitoring.generateMonitoringReport",
  });
...
export async function logMonitoringSummary(): Promise<void> {
  ...
  logger.info("Nora monitoring summary", {
    ...
    fn: "nora.monitoring.logMonitoringSummary",
  });
}
