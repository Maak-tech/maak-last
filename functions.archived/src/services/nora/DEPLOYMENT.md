...
NORA_MODEL=gpt-4o-mini
NORA_TIMEOUT_MS=10000
NORA_MAX_RETRIES=3

# Feature flags
NORA_USE_AI=true  # Set to false to disable AI globally
```
...
firebase functions:log --only nora
```
...
firebase functions:log --only nora --follow
```
...
# Verify nora.calls counter increasing

# 2. Check logs
firebase functions:log --only nora --limit 100
...
1. **nora.calls** - Total analysis calls
   - Alert if drops to 0 (service down)
   - Alert if spikes >2x normal (unexpected traffic)

2. **nora.failures** - Analysis failures
   - Alert if >5% of total calls
   - Page on-call if >10%

3. **nora.guardrail_blocks** - Validation blocks
   - Alert if >5% of total calls
   - Investigate if trending up

4. **nora.llm_timeouts** - LLM timeout errors
   - Alert if >10% of LLM calls
   - May need to increase timeout

5. **nora.duration.over_5s** - Slow analyses
...
| filter resource.function_name =~ 'nora.*'
...
- name: nora_high_failure_rate
  condition: |
    nora.failures / nora.calls > 0.05
  duration: 5m
  severity: warning
  action: notify_on_call

- name: nora_critical_failure_rate
  condition: |
    nora.failures / nora.calls > 0.10
  duration: 2m
  severity: critical
  action: page_on_call

- name: nora_high_timeout_rate
  condition: |
    nora.llm_timeouts / nora.llm_calls > 0.10
  duration: 5m
  severity: warning
  action: notify_team

- name: nora_guardrail_blocks_trending
  condition: |
    nora.guardrail_blocks / nora.calls > 0.05
  duration: 10m
  severity: info
  action: create_ticket

- name: nora_service_down
  condition: |
    rate(nora.calls[5m]) == 0
  duration: 5m
  severity: critical
  action: page_on_call
```
...
If Nora is causing critical issues:

```bash
# Option 1: Disable AI (use deterministic only)
firebase functions:config:set nora.enabled=false
firebase deploy --only functions
...
firebase functions:config:set nora.use_ai=false
firebase deploy --only functions
...
- Nora blocking critical health alerts
- PHI leak detected
...
|-------|----------|----------|
| High timeout rate | >10% LLM timeouts | Increase `NORA_TIMEOUT_MS` or switch to deterministic |
...
**On-Call Rotation**
- Primary: <on-call-primary@nuralix.ai>
- Secondary: <on-call-secondary@nuralix.ai>

**Escalation**
- Engineering Manager: <eng-manager@nuralix.ai>
- CTO: <cto@nuralix.ai>
...
```bash
firebase functions:log --only nora --limit 10
```
...
### Check Recent Errors
```bash
firebase functions:log --only nora --filter "severity>=ERROR" --limit 50
```
