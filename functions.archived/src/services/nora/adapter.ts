/**
 * Nora Backward Compatibility Adapter
 *
 * Bridges the old Nora API (from analyze.ts) with the new HIPAA-safe architecture.
...
import { type AlertContext, runNoraAnalysis } from "./index";
...
export interface NoraAnalysisInput {
  patientId: string;
  alert: AlertInfo;
 ...
 *
 * @deprecated Use runNoraAnalysis() from index.ts instead
 * This adapter is for backward compatibility only
 */
export async function analyze(
  input: NoraAnalysisInput
  ...
  try {
    ...
    // Convert new NoraOutput to old NoraAnalysisResult format
    const legacyResult: NoraAnalysisResult = {
