 ...
 *
 * Converts validated NoraOutput into concrete backend actions:
...
import type { BackendActions, NoraOutput } from "./types";
 ...
 */
function mapActionCodeToAutoActions(
...
export function mapToBackendActions(
  ...
  logger.info("Mapping NoraOutput to backend actions", {
    traceId,
    riskScore: output.riskScore,
    escalationLevel: output.escalationLevel,
    actionCode: output.recommendedActionCode,
    fn: "nora.outputMapper.mapToBackendActions",
  });

  // Determine if alert should be sent
  const sendAlert =
    output.escalationLevel !== EscalationLevel.NONE || output.riskScore >= 50;
...
/**
 * Format NoraOutput for storage/audit log
