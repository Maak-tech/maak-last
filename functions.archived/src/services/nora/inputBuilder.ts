 ...
 * Output (NoraInput) contains ZERO PHI and is safe to send to external LLMs.
 *
 * Transformations:
...
import type { AlertContext, NoraInput } from "./types";
...
function bucketVitalValue(
  ...
  const ranges: Record<
    string,
    { veryLow: number; low: number; high: number; veryHigh: number }
  > = {
 ...
 * @returns NoraInput with NO PHI
 */
export function buildNoraInput(
  ...
  logger.debug("Building AI-safe input", {
    ...
    fn: "nora.inputBuilder.buildNoraInput",
  });

  const input: NoraInput = {
    alertType: context.alertType,
...
/**
 * Build LLM prompt from NoraInput
 * This prompt is safe to send to external AI services
 */
export function buildAnalysisPrompt(input: NoraInput): string {
  const parts: string[] = [];

  parts.push(
    "Analyze the following health alert and provide a structured response."
  );
  parts.push("");
