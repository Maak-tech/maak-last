...
import { getRecentVitalsSummary } from "../../services/nora";
import {
  type AlertInfo,
...
export type ProcessVitalOptions = {
  ...
  openaiApiKey?: string; // OpenAI API key for Nora analysis
};
 ...
 * 5. Enrich with Nora analysis (optional)
...
export async function processVitalReading(
  ...
  try {
    ...
    // Step 5: Enrich with Nora analysis (optional, non-blocking)
    try {
      ...
      logger.warn("Nora analysis failed, continuing", noraError as Error, {
        traceId,
        alertId,
        patientId: reading.userId,
        fn: "processVitalReading",
