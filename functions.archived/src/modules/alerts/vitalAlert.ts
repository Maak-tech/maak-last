 ...
 * Handles vital alert creation, Nora enrichment, and notification to family admins
 */
...
import {
  type AlertInfo,
  analyze as noraAnalyze,
} from "../../services/nora/adapter";
import { enrichAlertWithAnalysis } from "../../services/nora/store";
 ...
 * Includes optional Nora AI analysis enrichment
 *
...
export async function sendVitalAlertToAdmins(
  ...
  try {
    ...
    // Optional: Nora AI Analysis enrichment
    // This is non-blocking - if it fails, alert still goes out
    try {
      logger.debug("Starting Nora analysis", {
        traceId,
        alertId,
        patientId: userId,
        fn: "sendVitalAlertToAdmins",
      ...
      // Run Nora analysis
      const analysis = await noraAnalyze({
        patientId: userId,
        alert: alertInfo,
      ...
      logger.info("Nora analysis completed and stored", {
        traceId,
        alertId,
      ...
      // Don't fail the whole alert if Nora fails
      logger.warn(
        "Nora analysis failed, continuing with alert",
        noraError as Error,
        {
          traceId,
          alertId,
    ...
    const notification = {
      ...
      data: {
        ...
        userId,
        userName,
        alertId, // Include alertId so app can fetch full alert with Nora analysis
      },
