...
function selectOpenAIKey(usePremiumKey: boolean): string {
  const noraKey = getSecretValue(NORA_API_KEY, process.env.NORA_API_KEY);
  const openaiKey = getSecretValue(OPENAI_API_KEY, process.env.OPENAI_API_KEY);

  const preferred = usePremiumKey ? noraKey : openaiKey;
  const fallback = usePremiumKey ? openaiKey : noraKey;
  const key = preferred || fallback;

  if (!key) {
    throw new https.HttpsError(
      "failed-precondition",
      "OpenAI is not configured on the server. Set Firebase Functions secret OPENAI_API_KEY (and optionally NORA_API_KEY), then redeploy."
    );
  }
...
export const openaiHealthCheck = onCall(
  {
    secrets: [OPENAI_API_KEY, NORA_API_KEY, REVENUECAT_SECRET_API_KEY],
  },
  async (request) => {
    requireAuth(request);
    ...
    logger.debug("OpenAI health check", {
      traceId,
      configured: Boolean(openaiKey || noraKey),
      hasAccess,
