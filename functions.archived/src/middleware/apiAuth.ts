...
function extractApiKey(req: Request): string | null {
  ...
  if (authHeader?.startsWith("Bearer ")) {
    ...
    if (token.startsWith("nk_live_")) return token;
  }

  const apiKeyHeader = req.headers["x-api-key"] as string | undefined;
  if (apiKeyHeader?.startsWith("nk_live_")) return apiKeyHeader;
...
export async function authenticateApiKey(
  ...
  if (!rawKey) {
    res.status(401).json({
      ...
      code: "unauthorized",
      hint: "Include Authorization: Bearer nk_live_<key> or X-API-Key header",
    });
    return false;
  }
