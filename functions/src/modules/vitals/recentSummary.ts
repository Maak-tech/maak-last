...
export interface VitalsSummary {
  heartRate?: { current: number; avg: number; trend: 'stable' | 'increasing' | 'decreasing' };
  bloodPressure?: { 
    systolic: { current: number; avg: number }; 
    diastolic: { current: number; avg: number };
    trend: 'stable' | 'increasing' | 'decreasing';
  };
  oxygenSaturation?: { current: number; avg: number; trend: 'stable' | 'increasing' | 'decreasing' };
  bodyTemperature?: { current: number; avg: number; trend: 'stable' | 'increasing' | 'decreasing' };
  weight?: { current: number; change: number; trend: 'stable' | 'increasing' | 'decreasing' };
  respiratoryRate?: { current: number; avg: number; trend: 'stable' | 'increasing' | 'decreasing' };
  [key: string]: any;
}
...
export async function getRecentVitalsSummary(
  ...
  try {
    ...
    for (const [type, readings] of Object.entries(vitalsByType)) {
      ...
      if (type === 'bloodPressure' && readings[0].systolic && readings[0].diastolic) {
