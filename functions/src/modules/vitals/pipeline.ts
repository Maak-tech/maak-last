  ...
  try {
    ...
    if (!skipPersistence) {
      ...
      await writeAuditRecord(
        ...
        'vital_created',
        reading.userId,
        { vitalId, vitalType: reading.type }
      );
    }
    ...
    await writeAuditRecord(
      ...
      'alert_created',
      reading.userId,
      { alertId, vitalId, severity: checkResult.severity }
    );
    ...
    try {
      ...
      const alertInfo: AlertInfo = {
        type: 'vital',
        severity: (checkResult.severity || 'warning') as 'critical' | 'warning' | 'info',
        title: alertMessage.title,
        body: alertMessage.message,
        data: {
          vitalType: reading.type,
