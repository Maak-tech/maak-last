...
function testNormalReadings() {
  ...
  assert.strictEqual(result1.isAlert, false, 'Normal heart rate should not alert');
  ...
  assert.strictEqual(result3.isAlert, false, 'Normal temperature should not alert');
...
function testWarningThresholds() {
  ...
  assert.strictEqual(result2.direction, 'high', 'Should indicate high direction');
...
function testCriticalThresholds() {
  ...
  assert.strictEqual(result1.severity, 'critical', 'Should be critical severity');
  ...
  assert.strictEqual(result2.severity, 'critical', 'Should be critical severity');
...
function testDuplicateSuppression() {
  ...
  assert.strictEqual(suppress1, false, 'Should not suppress when no recent alerts');
  ...
  const suppress2 = shouldSuppressAlert(newAlert2, recentAlerts2, 60 * 60 * 1000);
  assert.strictEqual(suppress2, true, 'Should suppress duplicate within 1 hour window');
  ...
  const suppress3 = shouldSuppressAlert(newAlert3, recentAlerts3, 60 * 60 * 1000);
  assert.strictEqual(suppress3, false, 'Should not suppress old alert outside window');
  ...
  assert.strictEqual(suppress4, false, 'Should not suppress different severity');
  ...
  assert.strictEqual(suppress5, false, 'Should not suppress different vital type');
...
function testSuppressionWindow() {
  ...
  assert.strictEqual(criticalWindow, 30 * 60 * 1000, 'Critical alerts: 30 min window');

  const warningWindow = getSuppressionWindow('warning');
  assert.strictEqual(warningWindow, 2 * 60 * 60 * 1000, 'Warning alerts: 2 hour window');
...
function testAlertMessages() {
  ...
  assert.strictEqual(msg1.title.includes('⚠️'), true, 'Warning should have warning emoji');
  ...
  const msg2 = createAlertMessage('oxygenSaturation', 85, '%', 'critical', 'low');
  assert.strictEqual(msg2.title.includes('🚨'), true, 'Critical should have critical emoji');
...
function testEdgeCases() {
  ...
  const atWarningLow = checkVitalBenchmark('heartRate', hr.alertThresholds.low.warning);
  assert.strictEqual(atWarningLow.isAlert, true, 'At warning threshold should alert');
  assert.strictEqual(atWarningLow.severity, 'warning');

  // Exactly at critical threshold (should trigger)
  const atCriticalLow = checkVitalBenchmark('heartRate', hr.alertThresholds.low.critical);
  assert.strictEqual(atCriticalLow.isAlert, true, 'At critical threshold should alert');
  assert.strictEqual(atCriticalLow.severity, 'critical');

  // Just above warning threshold (should NOT trigger if in normal range)
  const justAboveWarning = checkVitalBenchmark('heartRate', hr.alertThresholds.low.warning + 1);
  // This depends on normal range - if 51 is in normal range, no alert
  if (51 >= hr.normalRange.min && 51 <= hr.normalRange.max) {
    assert.strictEqual(justAboveWarning.isAlert, false, 'Just above warning threshold in normal range');
  }
