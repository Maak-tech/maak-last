...
export async function sendToUser(
  ...
  const traceId = options.traceId || Math.random().toString(36).substring(2, 15);
  ...
  try {
    ...
    const message = {
      ...
      android: {
        ...
        notification: {
          ...
          priority: options.priority === 'high' ? ('high' as const) : ('default' as const),
          channelId: type,
    ...
    // Record attempt
    const status: NotificationAttemptStatus = response.successCount > 0 ? 'sent' : 'failed';
    await recordNotificationAttempt(
      traceId,
      userId,
      type,
      status,
      tokens.length,
      response.failureCount > 0 ? `${response.failureCount} tokens failed` : undefined
    );
