 ...
 */
export function validateAlertContext(context: AlertContext, traceId: string): GuardrailResult {
  const errors: string[] = [];
  ...
  // Validate vital data
  if (context.vitalValue !== undefined && (context.vitalValue < 0 || context.vitalValue > 1000)) {
 ...
 */
export function validateAIResponse(response: RawAIResponse, traceId: string): GuardrailResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  if (!response.summary) {
    ...
    const diagnosticTerms = [
      ...
      'condition is', 'you have', 'patient has',
      'cancer', 'infection', 'diabetes', 'hypertension',
    ];
  ...
  const validActionCodes = [
    'MONITOR', 'CHECK_VITALS', 'RECHECK_IN_1H', 'RECHECK_IN_24H',
    'CONTACT_PATIENT', 'UPDATE_FAMILY', 'NOTIFY_CAREGIVER',
    'REVIEW_MEDICATIONS', 'REVIEW_HISTORY', 'ASSESS_SYMPTOMS',
    'SCHEDULE_CONSULTATION', 'CONSIDER_EMERGENCY', 'IMMEDIATE_ATTENTION',
  ];
  ...
  if (!response.recommendedActionCode) {
    ...
    errors.push(`recommendedActionCode must be one of: ${validActionCodes.join(', ')}`);
  }
  ...
  } else if (!validEscalationLevels.includes(response.escalationLevel.toLowerCase())) {
    errors.push(`escalationLevel must be one of: ${validEscalationLevels.join(', ')}`);
  }
...
export function applySafetyConstraints(
  ...
  if (severity === 'critical' && constrainedScore < 60) {
    logger.warn('Applying safety constraint: critical severity requires high risk score', {
      traceId,
  ...
  if (escalationLevel === 'emergency' && constrainedScore < 70) {
    logger.warn('Applying safety constraint: emergency escalation requires high risk score', {
      traceId,
      originalScore: riskScore,
      adjustedScore: 70,
      fn: 'zeina.guardrails.applySafetyConstraints',
  ...
  if (severity === 'info' && constrainedScore > 60) {
    logger.warn('Applying safety constraint: info severity should not have high risk score', {
      traceId,
      originalScore: riskScore,
      adjustedScore: 60,
      fn: 'zeina.guardrails.applySafetyConstraints',
  ...
  if (escalationLevel === 'none' && constrainedScore > 50) {
    logger.warn('Applying safety constraint: no escalation should not have high risk score', {
      traceId,
      originalScore: riskScore,
      adjustedScore: 50,
      fn: 'zeina.guardrails.applySafetyConstraints',
...
export function sanitizeToZeinaOutput(
  ...
  const actionCode = (response.recommendedActionCode as RecommendedActionCode) || 
                     RecommendedActionCode.MONITOR;

  // Normalize escalation level
  const escalationLevel = (response.escalationLevel?.toLowerCase() as EscalationLevel) || 
