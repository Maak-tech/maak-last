...
describe('Zeina Guardrails', () => {
  ...
  describe('validateAIResponse', () => {
    ...
    it('should reject diagnostic language in summary', () => {
      ...
      const result = validateAIResponse(response, 'trace_123');
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('diagnostic language'))).toBe(true);
    });
    ...
    it('should reject too long summary', () => {
      ...
      const result = validateAIResponse(response, 'trace_123');
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('under 200 characters'))).toBe(true);
    });
    ...
    it('should reject invalid action code', () => {
      ...
      const result = validateAIResponse(response, 'trace_123');
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('recommendedActionCode'))).toBe(true);
    });
    ...
    it('should reject invalid escalation level', () => {
      ...
      const result = validateAIResponse(response, 'trace_123');
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('escalationLevel'))).toBe(true);
    });
  });

  describe('applySafetyConstraints', () => {
    it('should enforce minimum risk score for critical severity', () => {
      const result = applySafetyConstraints(30, 'none', 'critical', 'trace_123');
      expect(result).toBeGreaterThanOrEqual(60);
    });

    it('should enforce minimum risk score for emergency escalation', () => {
      const result = applySafetyConstraints(40, 'emergency', 'warning', 'trace_123');
    ...
    it('should not modify risk score within valid range', () => {
      const result = applySafetyConstraints(50, 'caregiver', 'warning', 'trace_123');
      expect(result).toBe(50);
    });

    it('should cap risk score at 100', () => {
      const result = applySafetyConstraints(150, 'emergency', 'critical', 'trace_123');
  ...
  describe('sanitizeToZeinaOutput', () => {
    it('should create valid ZeinaOutput from AI response', () => {
      ...
      const output = sanitizeToZeinaOutput(response, context, 'trace_123', 'gpt-4o-mini');
