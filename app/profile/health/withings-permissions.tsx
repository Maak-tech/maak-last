/**
 * Withings Permissions Screen
 * Metric selection before requesting Withings OAuth
 */
...
export default function WithingsPermissionsScreen() {
  ...
  useEffect(() => {
    const loadMetrics = async () => {
      try {
        ...
        const availability = await withingsService.isAvailable();
        ...
        const metrics = getAvailableMetricsForProvider("withings");
  ...
  const handleConnect = async () => {
    ...
    try {
      setConnecting(true);
      await withingsService.startAuth(Array.from(selectedMetrics));

      Alert.alert(
        isRTL ? "تم الربط بنجاح" : "Successfully Connected",
        isRTL ? "تم ربط وايثينجز بنجاح. سيبدأ مزامنة البيانات قريباً." : "Withings connected successfully. Data sync will begin shortly.",
        [{ text: isRTL ? "موافق" : "OK", onPress: () => router.replace("/profile/health-integrations") }]
      );
    } catch (error: any) {
      Alert.alert(
        isRTL ? "فشل الربط" : "Connection Failed",
        error.message || (isRTL ? "فشل في ربط وايثينجز" : "Failed to connect Withings")
      );
    } finally {
      setConnecting(false);
    }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.connectSection}>
          <TouchableOpacity
            ...
            disabled={connecting}
            style={[styles.connectButton, { backgroundColor: "#3B82F6" }, connecting && styles.connectButtonDisabled]}
          >
            {connecting ? (
              <ActivityIndicator color="#FFFFFF" size="small" />
            ) : (
              <>
                <Text style={styles.connectButtonText}>{isRTL ? "ربط وايثينجز" : "Connect Withings"}</Text>
...
const styles = StyleSheet.create({
  ...
  infoSection: { alignItems: "center", paddingVertical: 24, paddingHorizontal: 20, backgroundColor: "rgba(59, 130, 246, 0.05)", borderRadius: 12, marginBottom: 24 },
  ...
  metricCheckboxChecked: { backgroundColor: "#3B82F6", borderColor: "#3B82F6" },
