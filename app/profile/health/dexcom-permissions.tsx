/**
 * Dexcom CGM Permissions Screen
 * Metric selection before requesting Dexcom OAuth
 */
...
export default function DexcomPermissionsScreen() {
  ...
  useEffect(() => {
    const loadMetrics = async () => {
      try {
        ...
        const availability = await dexcomService.isAvailable();
        ...
        const metrics = getAvailableMetricsForProvider("dexcom");
  ...
  const handleConnect = async () => {
    ...
    try {
      setConnecting(true);
      await dexcomService.startAuth(Array.from(selectedMetrics));

      Alert.alert(
        isRTL ? "تم الربط بنجاح" : "Successfully Connected",
        isRTL ? "تم ربط Dexcom CGM بنجاح. سيبدأ مزامنة البيانات قريباً." : "Dexcom CGM connected successfully. Real-time glucose data sync will begin shortly.",
        [{ text: isRTL ? "موافق" : "OK", onPress: () => router.replace("/profile/health-integrations") }]
      );
    } catch (error: any) {
      Alert.alert(
        isRTL ? "فشل الربط" : "Connection Failed",
        error.message || (isRTL ? "فشل في ربط Dexcom CGM" : "Failed to connect Dexcom CGM")
      );
    } finally {
      setConnecting(false);
    }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background.primary }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: theme.colors.text.primary }, isRTL && styles.rtlText]}>
          {isRTL ? "أذونات Dexcom CGM" : "Dexcom CGM Permissions"}
        </Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.infoSection}>
          <View style={[styles.infoIcon, { backgroundColor: isDark ? "rgba(16, 185, 129, 0.2)" : "rgba(16, 185, 129, 0.1)" }]}>
            <Info color="#10B981" size={24} />
          </View>
          <Text style={[styles.infoTitle, { color: theme.colors.text.primary }, isRTL && styles.rtlText]}>
            {isRTL ? "مراقبة السكر المستمرة" : "Continuous Glucose Monitoring"}
          </Text>
          <Text style={[styles.infoDescription, { color: theme.colors.text.secondary }, isRTL && styles.rtlText]}>
            {isRTL 
              ? "سيتم مزامنة قراءات السكر في الدم في الوقت الفعلي من جهاز Dexcom الخاص بك" 
              : "Real-time blood glucose readings will be synced from your Dexcom device"}
          </Text>
        </View>
        ...
        <View style={styles.connectSection}>
          <TouchableOpacity
            ...
            disabled={connecting}
            style={[styles.connectButton, { backgroundColor: "#10B981" }, connecting && styles.connectButtonDisabled]}
          >
            {connecting ? (
              <ActivityIndicator color="#FFFFFF" size="small" />
            ) : (
              <>
                <Text style={styles.connectButtonText}>{isRTL ? "ربط Dexcom CGM" : "Connect Dexcom CGM"}</Text>
...
const styles = StyleSheet.create({
  ...
  infoSection: { alignItems: "center", paddingVertical: 24, paddingHorizontal: 20, backgroundColor: "rgba(16, 185, 129, 0.05)", borderRadius: 12, marginBottom: 24 },
  ...
  metricCheckboxChecked: { backgroundColor: "#10B981", borderColor: "#10B981" },
