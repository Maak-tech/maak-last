 ...
 * Metric selection before requesting Garmin OAuth
 */
...
export default function GarminPermissionsScreen() {
  ...
  useEffect(() => {
    const loadMetrics = async () => {
      try {
        ...
        const availability = await garminService.isAvailable();
        ...
        // Get available metrics for Garmin
        const metrics = getAvailableMetricsForProvider("garmin");
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Start OAuth flow
      await garminService.startAuth(Array.from(selectedMetrics));

      // If we get here, OAuth was successful
      Alert.alert(
        isRTL ? "تم الربط بنجاح" : "Successfully Connected",
        isRTL
          ? "تم ربط جارمين كونكت بنجاح. سيبدأ مزامنة البيانات قريباً."
          : "Garmin Connect connected successfully. Data sync will begin shortly.",
        [
          {
            text: isRTL ? "موافق" : "OK",
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        >
          {isRTL ? "أذونات جارمين كونكت" : "Garmin Connect Permissions"}
        </Text>
