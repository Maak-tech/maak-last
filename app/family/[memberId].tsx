...
import GeneticRiskSummaryCard, {
  type GeneticRiskSummaryData,
} from "@/components/GeneticRiskSummaryCard";
...
export default function FamilyMemberHealthView() {
  ...
  const [memberGeneticSummary, setMemberGeneticSummary] =
    useState<GeneticRiskSummaryData | null>(null);
  ...
  // Load genetic risk summary from member's VHI (admin-only, best-effort)
  useEffect(() => {
    if (!memberId || (user?.role !== "admin" && user?.role !== "caregiver")) return;
    vhiService.getMemberVHI(memberId).then((vhi) => {
      if (!vhi?.data?.geneticBaseline?.hasGeneticData) return;
      const gb = vhi.data.geneticBaseline;
      setMemberGeneticSummary({
        conditions: gb.prsScores.map((p) => ({
          condition: p.condition,
          percentile: p.percentile,
          level: p.level,
        })),
        pharmacogenomicsAlerts: gb.pharmacogenomics.map((ph) => ({
          drug: ph.drug,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Genetic Risk Summary — visible to admins/caregivers when member has shared genetics */}
        {(user?.role === "admin" || user?.role === "caregiver") && (
          <View style={styles.section}>
            <GeneticRiskSummaryCard
              ...
              memberName={
                member?.firstName ??
                (isRTL ? "العضو" : "This member")
              }
              summary={memberGeneticSummary}
            />
          </View>
        )}
