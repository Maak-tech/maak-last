...
export default function AIAssistant() {
  ...
  const initializeChat = async () => {
    try {
      ...
      const welcomeMessage: AIMessage = {
        ...
        role: 'assistant',
        content: "Hello! I'm your personal health AI assistant. I have access to your health profile, medications, symptoms, and family information. How can I help you today?",
        timestamp: new Date(),
      };
  ...
  const saveMessageToSession = async (message: AIMessage) => {
    ...
    try {
      ...
      if (message.role === 'user' && currentMessages.filter(m => m.role === 'user').length === 0) {
  ...
  const handleSend = async () => {
    ...
    if (!apiKey) {
      Alert.alert('API Key Required', 'Please configure your OpenAI API key first.');
        ...
        >
          {isLoading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Loading your health context...</Text>
            </View>
          ) : (
            messages
