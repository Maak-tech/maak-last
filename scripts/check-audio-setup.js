...
try {
  ...
  const allPresent = deps['expo-av'] && deps['expo-device'] && deps['expo-file-system'] && deps['expo-constants'];
  console.log(`   ${allPresent ? '✓' : '✗'} All required packages ${allPresent ? 'found' : 'MISSING'}`);
} catch (error) {
...
const modulesToCheck = ['expo-av', 'expo-device', 'expo-file-system', 'expo-constants'];
modulesToCheck.forEach(mod => {
  const modPath = path.join(process.cwd(), 'node_modules', mod);
  const exists = fs.existsSync(modPath);
  console.log(`   ${exists ? '✓' : '✗'} ${mod} ${exists ? 'installed' : 'NOT INSTALLED'}`);
});
console.log();
