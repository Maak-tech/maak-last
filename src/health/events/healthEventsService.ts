...
export async function getUserHealthEvents(
  ...
  try {
    logger.debug("Fetching user health events", { userId, limitCount }, "healthEventsService");
...
export async function getActiveHealthEvents(userId: string): Promise<HealthEvent[]> {
  ...
  try {
    logger.debug("Fetching active health events", { userId }, "healthEventsService");
    ...
    logger.error("Failed to get active health events", error, "healthEventsService");
    return [];
  }
}
...
export async function getHealthEventsByStatus(
  ...
  try {
    logger.debug("Fetching health events by status", { userId, status }, "healthEventsService");

    const events = await getUserHealthEvents(userId, 200);
    const filteredEvents = events.filter(event => event.status === status);

    logger.info("Health events by status fetched", {
      userId,
      status,
      totalEvents: events.length,
    ...
    logger.error("Failed to get health events by status", error, "healthEventsService");
    return [];
  }
}
...
export async function getFamilyHealthEvents(
  ...
  try {
    if (userIds.length === 0) {
      logger.debug("No user IDs provided for family health events", {}, "healthEventsService");
      return [];
    }
    ...
    const durationMs = Date.now() - startTime;
    logger.error("Failed to get family health events", error, "healthEventsService");
    return [];
  }
}
