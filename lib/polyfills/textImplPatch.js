  ...
  try {
    ...
    function wrapWithForwardRef(component, displayName) {
      ...
      try {
        ...
        Wrapped.displayName =
          displayName || component.displayName || component.name || "Wrapped";
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Legacy polyfill performs broad runtime compatibility patching.
(function patchTextImplAndReanimated() {
  try {
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Predicate intentionally checks multiple runtime shapes.
    function isForwardRefComponent(component) {
      ...
      try {
        ...
        if (component.__isForwardRef === true) {
          return true;
        }
    ...
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Wrapper preserves compatibility/static properties across versions.
    function wrapWithForwardRef(component, displayName) {
      try {
    ...
    const wrappedTextImplFallback = wrapWithForwardRef(
      ReactNative.Text,
      "TextImpl"
    );
    ...
    function patchReanimatedModule(reanimatedModule) {
      if (!reanimatedModule?.exports) {
        return;
      }

      const reanimated =
        reanimatedModule.exports.default || reanimatedModule.exports;
      if (
        reanimated?.createAnimatedComponent &&
        !reanimated.createAnimatedComponent.__patchedByTextImplPatch
      ) {
        const originalCreateAnimatedComponent =
          reanimated.createAnimatedComponent.bind(reanimated);

        // Wrap createAnimatedComponent to PROACTIVELY wrap function components
        // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Interceptor must handle diverse component signatures defensively.
        reanimated.createAnimatedComponent = (component) => {
          try {
            if (!component) {
            ...
            const isTextRelated =
              ...
              (ReactNative.TextInput &&
                component === ReactNative.TextInput.TextImpl);
        ...
        if (
          reanimatedModule.exports.default &&
          reanimatedModule.exports.default !== reanimated
        ) {
          reanimatedModule.exports.default.createAnimatedComponent =
            reanimated.createAnimatedComponent;
