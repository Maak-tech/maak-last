    ...
    function patchReanimatedModule(reanimatedModule) {
      ...
      if (reanimated && reanimated.createAnimatedComponent && !reanimated.createAnimatedComponent.__patchedByTextImplPatch) {
        ...
        reanimated.createAnimatedComponent = function(component) {
          ...
          // If it's TextImpl or Text component, wrap it BEFORE calling createAnimatedComponent
          if ((isTextImpl || isTextComponent) && !isForwardRefComponent(component)) {
            const wrapped = wrapWithForwardRef(component, componentName || 'TextImpl');
            try {
              return originalCreateAnimatedComponent(wrapped);
            } catch (error) {
              // If wrapping still fails, return wrapped component anyway
          ...
          // For other components, try wrapping if they fail
          try {
            return originalCreateAnimatedComponent(component);
          } catch (error) {
            const errorMessage = error?.message || String(error || '');
