...
export const openMotionSettings = async () => {
  if (Platform.OS === "ios") {
    ...
    if (canOpen) {
      ...
      Alert.alert(
        "Open Settings",
        "Please go to Settings → Privacy & Security → Motion & Fitness → Nuralix and enable motion access."
      );
    }
  } else {
    // Android - open app settings
    const url = `package:${Constants.expoConfig?.android?.package || "com.nuralix.app"}`;
    const canOpen = await Linking.canOpenURL(url);
    if (canOpen) {
      await Linking.openURL(url);
    } else {
      Alert.alert(
        "Open Settings",
        "Please go to Settings → Apps → Nuralix → Permissions and enable Activity Recognition."
      );
    }
  }
};
