...
export const checkMotionAvailability = async (): Promise<MotionPermissionStatus> => {
  ...
  if (Platform.OS === "web") {
    console.log("[MotionPermissionService] ⚠️ Web platform - motion sensors not available");
    return {
      available: false,
      granted: false,
  ...
  try {
    ...
    console.log("[MotionPermissionService] 🔍 Checking DeviceMotion.isAvailableAsync()...");
    ...
    console.log("[MotionPermissionService] 📊 DeviceMotion available:", isAvailable);

    if (!isAvailable) {
      console.error("[MotionPermissionService] ❌ Motion sensors not available on this device");
      return {
        available: false,
        granted: false,
    ...
    console.log("[MotionPermissionService] 📂 Checking stored permission status...");
    ...
    console.log("[MotionPermissionService] 📂 Stored permission:", hasStoredPermission);
    ...
    console.log("[MotionPermissionService] ✅ Motion availability check complete:", result);
    return result;
  } catch (error: any) {
    console.error("[MotionPermissionService] ❌ Error checking motion availability:", error);
    return {
      available: false,
      granted: false,
...
export const requestMotionPermission = async (): Promise<boolean> => {
  ...
  try {
    ...
    console.log("[MotionPermissionService] 👂 Adding listener to trigger permission dialog...");
    const subscription = DeviceMotion.addListener(() => {
      // Just listening to trigger permission
      console.log("[MotionPermissionService] 📊 Sensor data received - permission likely granted");
    });
    ...
    console.error("[MotionPermissionService] ❌ Error requesting permission:", error);
    return false;
  }
};
...
export const hasMotionPermission = async (): Promise<boolean> => {
  try {
    ...
    console.log("[MotionPermissionService] 📂 Stored permission:", hasPermission);
    return hasPermission;
  } catch (error) {
    console.error("[MotionPermissionService] ❌ Error checking permission:", error);
    return false;
  }
};
