...
class RealtimeAgentService {
  ...
  private async getClientSecret(): Promise<string> {
    ...
    this.clientSecretInFlight = (async () => {
      const result = await api.post<{ clientSecret?: string; expiresAt?: number }>(
        "/api/nora/realtime-session",
        {
  ...
  async connect(customInstructions?: string): Promise<void> {
    ...
    if (!consent.consented) {
      throw new Error(
        "AI Data Sharing is disabled. Enable it in Profile > AI Data Sharing to use Nora voice features."
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Nora voice is not configured on the server.\n\n${message}\n\n` +
          "To fix this:\n" +
          "1. Set Firebase Functions secret OPENAI_API_KEY (and optionally NORA_API_KEY)\n" +
          "2. Deploy functions\n"
      );
    }
  ...
  private async processAudioQueue(): Promise<void> {
    ...
    try {
      ...
      const tempUri = `${fileSystem.cacheDirectory}nora_audio_${Date.now()}_${Math.random().toString(36).substring(7)}.wav`;
