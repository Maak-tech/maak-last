...
const REDIRECT_URI = "https://app.nuralix.ai/withings-callback";
...
export const withingsService = {
  ...
  handleRedirect: async (url: string): Promise<void> => {
    try {
      // Handle both web URLs and deep links (nuralix://)
      let urlObj: URL;
      if (url.startsWith("nuralix://")) {
        // Convert deep link to parseable URL format
        urlObj = new URL(url.replace("nuralix://", "https://nuralix.app/"));
      } else {
        urlObj = new URL(url);
      }
