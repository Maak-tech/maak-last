...
const DRUG_INTERACTIONS_DB: Record<string, {
  ...
  Warfarin: {
    interactsWith: ["Aspirin", "Ibuprofen", "Naproxen", "Paracetamol", "Atorvastatin", "Simvastatin"],
    severity: "major",
    description: "Warfarin interacts with many medications and can cause serious bleeding",
    effects: ["Increased risk of bleeding", "Bruising", "Prolonged bleeding time"],
    recommendations: ["Monitor INR regularly", "Avoid NSAIDs", "Consult doctor before adding new medications"],
  },
  Aspirin: {
    interactsWith: ["Warfarin", "Ibuprofen", "Naproxen", "Metformin"],
    severity: "major",
    description: "Aspirin can increase bleeding risk when taken with blood thinners",
  ...
  Ibuprofen: {
    ...
    severity: "moderate",
    description: "Ibuprofen can interact with blood thinners and other medications",
  ...
  Lisinopril: {
    ...
    recommendations: ["Monitor potassium levels", "Avoid potassium supplements"],
  },
  Enalapril: {
    interactsWith: ["Potassium supplements", "Diuretics", "Lithium"],
    severity: "moderate",
  ...
  Sertraline: {
    interactsWith: ["MAO inhibitors", "Blood thinners", "Other antidepressants"],
    severity: "major",
...
class MedicationInteractionService {
    ...
    if (med1Key) {
      ...
      for (const interactingMed of med1Data.interactsWith) {
        if (
          ...
          med2Normalized.includes(this.normalizeMedicationName(interactingMed)) ||
    ...
    if (med2Key) {
      ...
      for (const interactingMed of med2Data.interactsWith) {
        if (
          ...
          med1Normalized.includes(this.normalizeMedicationName(interactingMed)) ||
  ...
  async checkNewMedicationInteraction(
    ...
    try {
      const existingMedications = await medicationService.getUserMedications(userId);
   ...
   */
  getSeverityLabel(severity: DrugInteraction["severity"], isRTL: boolean = false): string {
    const labels = {
