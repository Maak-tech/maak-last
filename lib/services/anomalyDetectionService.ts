...
class AnomalyDetectionService {
   ...
   */
  private getAnomalyCollection(userId: string) {
    return collection(db, "users", userId, "anomalies");
  }

  // ─── Baseline Management ──────────────────────────────────────────────

  /**
   * Refresh baseline for a vital type by querying Firestore directly
   * (not relying on in-memory buffer which is lost on restart)
   */
  async refreshBaseline(
    ...
    try {
      ...
      const readings: VitalReading[] = snapshot.docs
        .map((d) => {
          const data = d.data();
          return {
            type: vitalType,
  ...
  async enrichAnomalyContext(
    ...
    // Check historical frequency (how often this anomaly type occurred in last 30 days)
    ...
    try {
      ...
      const q = query(
        this.getAnomalyCollection(userId),
        where("vitalType", "==", anomaly.vitalType),
        where("timestamp", ">=", Timestamp.fromDate(thirtyDaysAgo)),
        limit(50)
      );
      const snapshot = await getDocs(q);
      historicalFrequency = snapshot.size;
    } catch {
      // Silently fail
    }
  ...
  async checkAndPersistAnomaly(
    ...
    try {
      ...
      try {
        ...
        const snapshot = await getDocs(q);
        const otherReadings = snapshot.docs
          .map((d) => {
            const data = d.data();
            return {
              type: data.type,
      ...
      const docRef = await addDoc(this.getAnomalyCollection(userId), {
        ...anomaly,
        id: undefined,
        timestamp: Timestamp.fromDate(anomaly.timestamp),
      });
      anomaly.id = docRef.id;
  ...
  async getAnomalyHistory(
    ...
    try {
      ...
      if (vitalType) {
        constraints.unshift(where("vitalType", "==", vitalType));
      }

      const q = query(this.getAnomalyCollection(userId), ...constraints);
      const snapshot = await getDocs(q);
  ...
  async acknowledgeAnomaly(userId: string, anomalyId: string): Promise<void> {
    try {
      const docRef = doc(this.getAnomalyCollection(userId), anomalyId);
      await setDoc(docRef, { acknowledged: true }, { merge: true });
    } catch (error) {
      logger.error(
        "Failed to acknowledge anomaly",
        { userId, anomalyId, error },
        "AnomalyDetection"
      );
    }
  }
