...
// ─── Single batch fetch (replaces 6 individual Firestore queries) ─────────────
//
// Fetches all vitals in the 30-day window via the REST API, then filters to the
// recovery vital types in-memory. Equivalent to the old Firestore `in` query
// without the 10-item operator limit.
...
async function fetchAllRecoveryVitals(userId: string): Promise<VitalsByType> {
  ...
  const raw = await api.get<Record<string, unknown>[]>(
    `/api/health/vitals?from=${cutoff.toISOString()}&limit=500`
  );
  ...
  for (const d of (Array.isArray(raw) ? raw : [])) {
    ...
    byType[type].push({ value, timestamp });
  }
  return byType;
}
...
class RecoveryScoreService {
  ...
  async calculateRecoveryScore(userId: string): Promise<RecoveryScoreResult> {
    ...
    try {
      ...
      console.warn('[recoveryScore] calculateRecoveryScore failed, returning fallback:', err instanceof Error ? err.message : String(err));
