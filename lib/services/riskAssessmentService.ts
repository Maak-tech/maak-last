...
const RISK_FACTORS = {
  ...
  family_heart_disease: { weight: 30, category: "genetic" as const, modifiable: false },
  family_diabetes: { weight: 25, category: "genetic" as const, modifiable: false },
  family_cancer: { weight: 20, category: "genetic" as const, modifiable: false },
  family_hypertension: { weight: 15, category: "genetic" as const, modifiable: false },
  ...
  excessive_alcohol: { weight: 20, category: "lifestyle" as const, modifiable: true },
  ...
  high_cholesterol: { weight: 20, category: "medical" as const, modifiable: true },
  ...
  high_blood_pressure: { weight: 25, category: "medical" as const, modifiable: true },
  high_blood_sugar: { weight: 20, category: "medical" as const, modifiable: true },
  irregular_heart_rate: { weight: 15, category: "medical" as const, modifiable: true },
  frequent_symptoms: { weight: 10, category: "medical" as const, modifiable: true },
...
const CONDITION_SPECIFIC_RISKS = {
  cardiovascular: {
    ...
    factors: ["family_heart_disease", "hypertension", "high_cholesterol", "smoking", "diabetes", "obesity", "sedentary"],
    screening: ["Regular blood pressure checks", "Cholesterol screening", "EKG if indicated"],
    prevention: ["Heart-healthy diet", "Regular exercise", "Weight management", "Quit smoking", "Stress management"]
  },
  diabetes: {
    conditions: ["type 2 diabetes", "diabetic complications"],
    factors: ["family_diabetes", "obesity", "sedentary", "poor_diet", "age_45_64", "age_65_plus"],
    screening: ["Annual blood glucose screening", "HbA1c testing"],
    prevention: ["Weight management", "Regular exercise", "Healthy diet", "Regular health screenings"]
  },
  cancer: {
    conditions: ["various cancers"],
    factors: ["family_cancer", "smoking", "obesity", "age_45_64", "age_65_plus"],
    screening: ["Age-appropriate cancer screenings", "Regular check-ups"],
    prevention: ["Quit smoking", "Healthy diet", "Regular exercise", "Limit alcohol", "Sun protection"]
  },
  respiratory: {
    ...
    prevention: ["Quit smoking", "Avoid pollutants", "Regular exercise", "Healthy diet"]
  },
  mental_health: {
    ...
    prevention: ["Stress management", "Regular exercise", "Social connections", "Healthy sleep habits"]
  }
};
...
class RiskAssessmentService {
  ...
  async generateRiskAssessment(userId: string): Promise<HealthRiskAssessment> {
    ...
    // Determine timeline
    const timeline = this.determineAssessmentTimeline(overallRiskScore, riskFactors);
  ...
  private async assessRiskFactors(
    ...
    if (age >= 65) {
      riskFactors.push(this.createRiskFactor("age_65_plus", RISK_FACTORS.age_65_plus));
    } else if (age >= 45) {
      riskFactors.push(this.createRiskFactor("age_45_64", RISK_FACTORS.age_45_64));
    } else if (age >= 30) {
      riskFactors.push(this.createRiskFactor("age_30_44", RISK_FACTORS.age_30_44));
    }
    ...
    // Lifestyle risks
    const lifestyleRisks = this.assessLifestyleRisks(user, symptoms, medications);
  ...
  private calculateOverallRiskScore(riskFactors: RiskFactor[]): number {
    ...
    const weightedScore = riskFactors.reduce((sum, factor) => {
      const weight = RISK_FACTORS[factor.id as keyof typeof RISK_FACTORS]?.weight || 10;
  ...
  private generatePreventiveRecommendations(
    ...
    conditionRisks
      ...
      .forEach(risk => {
        risk.preventiveMeasures.forEach(measure => recommendations.add(measure));
        if (risk.screeningRecommendations) {
          risk.screeningRecommendations.forEach(screening => recommendations.add(screening));
        }
      });
    ...
    if (highRiskFactors.length > 0) {
      recommendations.add("Consult with healthcare provider about your risk factors");
  ...
  private createRiskFactor(
    id: string,
    config: { weight: number; category: RiskFactor["category"]; modifiable: boolean }
  ): RiskFactor {
    ...
    return {
      ...
      riskLevel: config.weight > 30 ? "high" : config.weight > 20 ? "moderate" : "low",
  ...
  private getRiskFactorDescription(id: string): string {
    const descriptions: Record<string, string> = {
      age_65_plus: "Advanced age is a significant risk factor for many health conditions",
      family_heart_disease: "Family history increases genetic predisposition to heart disease",
      smoking: "Smoking significantly increases risk of heart disease, cancer, and respiratory conditions",
      hypertension: "High blood pressure increases risk of heart disease and stroke",
      diabetes: "Diabetes affects multiple organ systems and increases cardiovascular risk"
    };
  ...
  private getRiskFactorEvidence(id: string): string {
    const evidence: Record<string, string> = {
      age_65_plus: "Based on epidemiological studies showing age-related risk increase",
      smoking: "Supported by extensive research linking smoking to multiple health conditions",
  ...
  private getRiskFactorRecommendations(id: string): string[] | undefined {
    const recommendations: Record<string, string[]> = {
      smoking: ["Quit smoking", "Seek smoking cessation support", "Avoid secondhand smoke"],
      obesity: ["Weight management through diet and exercise", "Consult nutritionist"],
      sedentary: ["Aim for 150 minutes of moderate exercise weekly", "Incorporate daily activity"],
      hypertension: ["Regular blood pressure monitoring", "Dietary changes (DASH diet)", "Medication adherence"],
      high_cholesterol: ["Heart-healthy diet", "Regular exercise", "Medication if prescribed"]
    };
  ...
  private assessFamilyHistoryRisks(familyHistory: MedicalHistory[]): RiskFactor[] {
    const risks: RiskFactor[] = [];
    const familyConditions = familyHistory.map(h => h.condition.toLowerCase());

    if (familyConditions.some(c => c.includes("heart") || c.includes("cardiac"))) {
      risks.push(this.createRiskFactor("family_heart_disease", RISK_FACTORS.family_heart_disease));
    }
    if (familyConditions.some(c => c.includes("diabetes"))) {
      risks.push(this.createRiskFactor("family_diabetes", RISK_FACTORS.family_diabetes));
    }
    if (familyConditions.some(c => c.includes("cancer"))) {
      risks.push(this.createRiskFactor("family_cancer", RISK_FACTORS.family_cancer));
    }
    if (familyConditions.some(c => c.includes("hypertension"))) {
      risks.push(this.createRiskFactor("family_hypertension", RISK_FACTORS.family_hypertension));
    }

    return risks;
  }

  private assessMedicalHistoryRisks(medicalHistory: MedicalHistory[]): RiskFactor[] {
    const risks: RiskFactor[] = [];
    ...
    if (conditions.some(c => c.includes("hypertension"))) {
      risks.push(this.createRiskFactor("hypertension", RISK_FACTORS.hypertension));
    }
    if (conditions.some(c => c.includes("cholesterol"))) {
      risks.push(this.createRiskFactor("high_cholesterol", RISK_FACTORS.high_cholesterol));
    }
    if (conditions.some(c => c.includes("heart"))) {
      risks.push(this.createRiskFactor("heart_disease", RISK_FACTORS.heart_disease));
    }
  ...
  private assessCurrentHealthRisks(
    ...
    if (highBPReadings.length > bpReadings.length * 0.5) {
      risks.push(this.createRiskFactor("high_blood_pressure", RISK_FACTORS.high_blood_pressure));
    }
    ...
    if (recentSymptoms.length > 10) {
      risks.push(this.createRiskFactor("frequent_symptoms", RISK_FACTORS.frequent_symptoms));
    }
  ...
  private scoreToRiskLevel(score: number): "low" | "moderate" | "high" | "very_high" {
  ...
  private determineAssessmentTimeline(
    ...
    const highRiskFactors = riskFactors.filter(f => f.riskLevel === "high" || f.riskLevel === "very_high");
    const criticalSymptoms = riskFactors.some(f => f.id === "high_blood_pressure" || f.id === "frequent_symptoms");
