...
import type { VHI } from "./vhiService";
...
class SmartNotificationService {
  ...
  async generateComprehensiveNotifications(
    ...
    // ── VHI-based risk alerts (prepended — highest priority, overrides wellness ──
    // VHI alerts surface critical health signals and take precedence over tips.
    if (vhi) {
      const vhiAlerts = this.generateVhiAlerts(vhi);
      // Critical + high VHI alerts go to the FRONT of the queue (highest priority)
      const criticalVhiAlerts = vhiAlerts.filter(
        (a) => a.priority === "critical" || a.priority === "high"
      );
      const normalVhiAlerts = vhiAlerts.filter(
        (a) => a.priority !== "critical" && a.priority !== "high"
      );
      notifications.unshift(...criticalVhiAlerts);
      notifications.push(...normalVhiAlerts);
    }
  ...
  private async scheduleOptimizedNotification(
    ...
    try {
      ...
      const hasDuplicate = allScheduled.some((scheduled) => {
        const scheduledRecord = asRecord(scheduled);
        const scheduledContent = asRecord(scheduledRecord?.content);
        if (!scheduledContent) {
          return false;
        }
        ...
        if (scheduledContentKey !== contentKey) {
          return false;
        }

        // Check if time is within window
        const trigger = scheduledRecord?.trigger;
        if (trigger) {
          const triggerDate =
  ...
  generateDailyWellnessCheckin(
    ...
    const notifications: SmartNotification[] = [];
    const _t = i18n.t.bind(i18n); // Translation function
  ...
  generateMedicationCheckins(
    ...
    const morningMeds = medications.filter((med) =>
      med.reminders?.some((reminder) => {
        const hour = Number.parseInt(reminder.time.split(":")[0], 10);
  ...
  generateVitalSignsCheckins(userStats: UserStats): SmartNotification[] {
    ...
    const daysSinceBP = lastBP
      ? Math.floor((Date.now() - lastBP.getTime()) / (24 * 60 * 60 * 1000))
      : 30;
    ...
    const daysSinceWeight = lastWeight
      ? Math.floor((Date.now() - lastWeight.getTime()) / (24 * 60 * 60 * 1000))
      : 60;
    ...
    if (userStats.userProfile?.age && userStats.userProfile.age > 65) {
      ...
      const daysSinceTemp = lastTemp
        ? Math.floor((Date.now() - lastTemp.getTime()) / (24 * 60 * 60 * 1000))
        : 30;
    ...
    if (userStats.healthConditions?.includes("diabetes")) {
      ...
      const daysSinceBloodSugar = lastBloodSugar
        ? Math.floor(
            (Date.now() - lastBloodSugar.getTime()) / (24 * 60 * 60 * 1000)
          )
        : 7;
   ...
   */
  generateExerciseActivityCheckins(_userStats: UserStats): SmartNotification[] {
    const notifications: SmartNotification[] = [];
    const _t = i18n.t.bind(i18n);
   ...
   */
  generateSleepCheckins(_userStats: UserStats): SmartNotification[] {
    const notifications: SmartNotification[] = [];
    const _t = i18n.t.bind(i18n);
  ...
  private calculateUserStats(
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (activityDates.has(dateString)) {
        tempStreak++;
        if (i === 0) {
          currentStreak = tempStreak; // Current streak if today has activity
        }
      } else {
   ...
   */
  private getFamilyMembers(_userId: string): FamilyMemberHealth[] {
    return [
      {
        id: "member1",
   ...
   */
  private getCaregiverCoordinationNeeds(_userId: string): {
  ...
  generateConditionSpecificReminders(
    ...
    if (conditions.includes("diabetes")) {
      ...
      const daysSinceBloodSugar = lastBloodSugarCheck
        ? Math.floor(
            (Date.now() - lastBloodSugarCheck.getTime()) / (24 * 60 * 60 * 1000)
          )
        : 30;
    ...
    if (conditions.includes("hypertension")) {
      ...
      const daysSinceBP = lastBPCheck
        ? Math.floor(
            (Date.now() - lastBPCheck.getTime()) / (24 * 60 * 60 * 1000)
          )
        : 30;
    ...
    if (conditions.some((c) => ["asthma", "copd", "bronchitis"].includes(c))) {
      ...
      const daysSinceRespiratory = lastRespiratoryCheck
        ? Math.floor(
            (Date.now() - lastRespiratoryCheck.getTime()) /
              (24 * 60 * 60 * 1000)
          )
        : 30;
      ...
      if (daysSinceMoodLog >= 2) {
        notifications.push({
          ...
          quickActions: [
            ...
            {
              label: t("quickActionTalkToNora", "🤖 Talk to Nora"),
              action: "open_nora",
            },
            {
  ...
  generateVitalSignPrompts(userStats: UserStats): SmartNotification[] {
    ...
    if (userStats.userProfile?.age && userStats.userProfile.age > 50) {
      ...
      const daysSinceWeight = lastWeightCheck
        ? Math.floor(
            (Date.now() - lastWeightCheck.getTime()) / (24 * 60 * 60 * 1000)
          )
        : 30;
    ...
    if (userStats.userProfile?.age && userStats.userProfile.age > 65) {
      ...
      const daysSinceTemp = lastTempCheck
        ? Math.floor(
            (Date.now() - lastTempCheck.getTime()) / (24 * 60 * 60 * 1000)
          )
        : 30;
  ...
  generateAchievementCelebrations(userStats: UserStats): SmartNotification[] {
    ...
    newAchievements.forEach((achievement) => {
      let _achievementType = "";
      let emoji = "🏆";

      // Determine achievement type and emoji
      if (achievement.type === "streak") {
        _achievementType = t("achievementTypeStreak", "Streak");
        emoji = "🔥";
      } else if (achievement.type === "compliance") {
        _achievementType = t("achievementTypeCompliance", "Compliance");
        emoji = "💊";
      } else if (achievement.type === "consistency") {
        _achievementType = t("achievementTypeConsistency", "Consistency");
        emoji = "📅";
      } else if (achievement.type === "milestone") {
        _achievementType = t("achievementTypeMilestone", "Milestone");
        emoji = "🎯";
      }
  ...
  // ── VHI-aware notification routing ──────────────────────────────────────────

  /**
   * Generate risk-aware notifications derived from the user's Virtual Health
   * Identity. Call from `generateComprehensiveNotifications` when a VHI object
   * is available.
   *
   * Routing rules:
   *   - compositeRisk ≥ 85 → CRITICAL caregiver alert
   *   - compositeRisk ≥ 75 → HIGH risk nudge to patient + caregiver alert
   *   - compositeRisk ≥ 60 → NORMAL risk awareness nudge
   *   - fallRisk ≥ 70      → HIGH fall risk notification
   *   - adherenceRisk ≥ 60 → HIGH adherence reminder (if not already in queue)
   *   - First HIGH-impact declining factor → NORMAL explanation prompt
   */
  generateVhiAlerts(vhi: VHI | null): SmartNotification[] {
    if (!vhi?.data) return [];
    ...
    const compositeRisk = rs.compositeRisk ?? 0;
    const fallRisk = rs.fallRisk?.score ?? 0;
    const adherenceRisk = rs.adherenceRisk?.score ?? 0;
    const topDeclining = vhi.data.decliningFactors?.[0] ?? null;
    ...
    if (compositeRisk >= 85) {
      notifications.push({
        id: `vhi-critical-${vhi.userId}-${Date.now()}`,
        title: isArabic ? "⚠️ تنبيه صحي عاجل" : "⚠️ Urgent Health Alert",
        body: isArabic
          ? `مؤشر صحتك في وضع حرج (${compositeRisk}/100). تواصل مع فريق الرعاية اليوم.`
          : `Your health identity score is critical (${compositeRisk}/100). Contact your care team today.`,
        type: "alert",
        priority: "critical",
        scheduledTime: new Date(),
        data: { type: "vhi_critical", compositeRisk },
        quickActions: [
          { label: isArabic ? "اسأل نورا" : "Ask Nora", action: "open_nora" },
          { label: isArabic ? "اتصل بمقدم الرعاية" : "Call Caregiver", action: "call_caregiver" },
        ],
      });
    } else if (compositeRisk >= 75) {
      notifications.push({
        id: `vhi-high-risk-${vhi.userId}-${Date.now()}`,
        title: isArabic ? "📊 هويتك الصحية تحتاج اهتمامك" : "📊 Your Health Identity Needs Attention",
        body: isArabic
          ? `درجة المخاطر المركبة ${compositeRisk}/100 — اعرف ما يحدث.`
          : `Composite risk score ${compositeRisk}/100 — find out what's happening.`,
        type: "alert",
        priority: "high",
        scheduledTime: new Date(),
        data: { type: "vhi_high_risk", compositeRisk },
        quickActions: [
          { label: isArabic ? "عرض هويتي الصحية" : "View My Health Identity", action: "open_vhi" },
        ],
      });
    } else if (compositeRisk >= 60) {
      notifications.push({
        id: `vhi-moderate-risk-${vhi.userId}-${Date.now()}`,
        title: isArabic ? "💡 ملاحظة صحية" : "💡 Health Note",
        body: isArabic
          ? `هويتك الصحية تُظهر بعض التغييرات هذا الأسبوع.`
          : `Your health identity shows some changes this week.`,
        type: "wellness_checkin",
        priority: "normal",
        scheduledTime: new Date(),
        data: { type: "vhi_moderate_risk", compositeRisk },
        quickActions: [
          { label: isArabic ? "معرفة المزيد" : "Learn More", action: "open_nora" },
        ],
      });
    }

    // ── Fall risk alert ───────────────────────────────────────────────────────
    if (fallRisk >= 70 && compositeRisk < 85) {
      notifications.push({
        id: `vhi-fall-risk-${vhi.userId}-${Date.now()}`,
        title: isArabic ? "🚨 خطر السقوط مرتفع" : "🚨 Elevated Fall Risk",
        body: isArabic
          ? `رُصد خطر سقوط مرتفع (${fallRisk}/100). تأكد من سلامة بيئتك.`
          : `Fall risk is elevated (${fallRisk}/100). Ensure a safe environment around you.`,
        type: "alert",
        priority: fallRisk >= 85 ? "critical" : "high",
        scheduledTime: new Date(),
        data: { type: "vhi_fall_risk", fallRisk },
        quickActions: [
          { label: isArabic ? "اسأل نورا" : "Ask Nora", action: "open_nora" },
        ],
      });
    ...
    // ── Adherence risk alert (supplement medication-specific nudges) ──────────
    if (adherenceRisk >= 60 && compositeRisk < 85) {
      notifications.push({
        id: `vhi-adherence-risk-${vhi.userId}-${Date.now()}`,
        title: isArabic ? "💊 تنبيه الالتزام بالدواء" : "💊 Medication Adherence Alert",
        body: isArabic
          ? `الالتزام بالدواء منخفض (~${Math.round(100 - adherenceRisk)}%). هذا يؤثر على صحتك المركبة.`
          : `Medication adherence is low (~${Math.round(100 - adherenceRisk)}%). This is affecting your composite health score.`,
        type: "medication",
        priority: "high",
        scheduledTime: new Date(),
        data: { type: "vhi_adherence_risk", adherenceRisk },
        quickActions: [
          { label: isArabic ? "تعيين تذكير" : "Set Reminder", action: "set_medication_reminder" },
          { label: isArabic ? "تسجيل جرعة" : "Log Dose", action: "log_medication" },
        ],
      });
    }

    // ── Top declining factor explanation prompt ───────────────────────────────
    if (topDeclining?.impact === "high" && compositeRisk < 75) {
      notifications.push({
        id: `vhi-declining-${vhi.userId}-${Date.now()}`,
        title: isArabic ? "📉 عامل يخفض صحتك" : "📉 Health Factor to Address",
        body: isArabic
          ? topDeclining.factor
          : topDeclining.factor,
        type: "health_tip",
        priority: "normal",
        scheduledTime: new Date(),
        data: { type: "vhi_declining_factor", factor: topDeclining.factor },
        quickActions: [
          { label: isArabic ? "اسأل نورا" : "Ask Nora about this", action: "open_nora" },
        ],
      });
    }
