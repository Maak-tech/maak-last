...
import type { VHIAction, VirtualHealthIdentity } from "@/types/vhi";
...
export type CaregiverDashboardData = {
  ...
  // VHI fields — populated from Neon API when available
  vhiScore?: number;
  vhiTrajectory?: "worsening" | "stable" | "improving";
  ...
  topDecliningFactors?: Array<{
    factor: string;
    impact: "high" | "medium" | "low";
    recommendation: string;
  }>;
  pendingVHIActions?: VHIAction[];
...
class CaregiverDashboardService {
  ...
  async getMemberDashboardData(
    ...
    try {
      ...
      // Fetch VHI from new Neon API (best-effort)
      let vhiFields: Partial<CaregiverDashboardData> = {};
      try {
        const vhiRes = await api.get<{ data: VirtualHealthIdentity } | null>(
          `/api/vhi/${member.id}`
        );
        if (vhiRes?.data) {
          ...
          const vhiTrajectory: CaregiverDashboardData["vhiTrajectory"] =
            highDeclines >= 2 ? "worsening" : highDeclines === 1 ? "stable" : "improving";

          if (compositeRisk > 60) {
            attentionReasons.push(`VHI composite risk: ${compositeRisk}/100`);
          }
