...
/**
 * No anomalies endpoint exists yet — return an empty map immediately.
 */
...
export async function getPredictiveForecast(
  ...
  try {
    ...
    const [symptomsRaw, medsRaw, moodsRaw] = await Promise.all([
      api.get<Record<string, unknown>[]>(
        `/api/health/symptoms?from=${cutoff.toISOString()}&limit=200`
      ),
      api.get<Record<string, unknown>[]>("/api/health/medications?limit=50"),
      api
        .get<Record<string, unknown>[]>(
          `/api/health/moods?from=${cutoff.toISOString()}&limit=200`
        )
      ...
      (d) =>
        ({
          ...
          timestamp: d.recordedAt
            ? new Date(d.recordedAt as string)
            : new Date(),
        }) as Symptom
    );
    ...
    const medications: Medication[] = (medsRaw ?? [])
      .map((m) => ({
        ...
        dosage: (m.dosage as string | undefined) ?? "",
        isActive: m.isActive as boolean,
        frequency: m.frequency as string,
        startDate: m.startDate ? new Date(m.startDate as string) : new Date(),
        reminders: (m.reminders as Medication["reminders"]) ?? [],
      }))
      .filter((m) => m.isActive);
    ...
    const moods: Mood[] = (moodsRaw ?? []).map(
      (d) =>
        ({
          ...
          intensity: d.intensity as number,
          timestamp: d.recordedAt
            ? new Date(d.recordedAt as string)
            : new Date(),
        }) as Mood
