/**
 * Nora Actions Service
 *
 * This service handles all automated tasks that Nora can perform on behalf of the user.
 * Like Siri, Nora can understand natural language requests and execute actions.
 *
 * Supported Actions:
...
export type ActionResult = {
  ...
  message: string;
  data?: unknown;
  speakableResponse: string; // What Nora should say to the user
};
...
class NoraActionsService {
  ...
  async logVitalSign(
    ...
    try {
      ...
      // Save vital via API (userId resolved server-side from session)
  ...
  async requestCheckIn(reason?: string): Promise<ActionResult> {
    try {
      ...
      await alertService.createAlert({
        ...
        message: reason || "Nora requested a health check-in",
  ...
  async logMood(
    ...
    try {
      ...
      // Save mood via API (userId resolved server-side from session)
...
export const noraActionsService = new NoraActionsService();
export default noraActionsService;
