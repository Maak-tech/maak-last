 ...
 * persists discoveries via REST API (best-effort), detects new/changed discoveries,
...
class CorrelationDiscoveryService {
  ...
  async refreshDiscoveries(
    ...
    try {
      ...
      const existing = await this.loadDiscoveries(userId);
      ...
      for (let i = 0; i < insight.correlationResults.length; i++) {
        ...
        if (existingDoc) {
          ...
          await this.saveDiscovery(userId, updated);
          ...
          await this.saveDiscovery(userId, discovery);
          newCount++;
  ...
  async getDiscoveries(
    ...
    let discoveries = await this.loadDiscoveries(userId);
  ...
  /**
   * Get discoveries that are newer than a given date (for Nora proactive messaging)
   */
  ...
  async markAsSeen(userId: string, discoveryId: string): Promise<void> {
    try {
      // No dedicated endpoint — update in-memory cache only
  ...
  private async loadDiscoveries(userId: string): Promise<HealthDiscovery[]> {
    // No dedicated discoveries GET endpoint — return empty array; in-memory cache serves subsequent requests
    return [];
  }
  ...
  private async saveDiscovery(
    ...
    // Best-effort: record discovery as a timeline event
    try {
      await api.post("/api/health/timeline", {
        ...
        lastUpdatedAt: discovery.lastUpdatedAt.toISOString(),
