...
class SharedMedicationScheduleService {
  ...
  async getFamilyMedicationSchedules(
    ...
    const loadPromise = (async () => {
      try {
        ...
        console.warn('[sharedMedicationSchedule] Failed to load family schedule:', error instanceof Error ? error.message : String(error));
        return [];
      } finally {
    ...
    try {
      ...
      console.warn('[sharedMedicationSchedule] Family schedule promise failed:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }
  ...
  async getMemberMedicationSchedule(
    ...
    try {
      ...
      console.warn('[sharedMedicationSchedule] getMemberMedicationSchedule failed:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }

  /**
   * Mark medication as taken (for caregivers or self).
   *
   * Replaces direct Firestore `updateDoc` with
   * PATCH /api/health/medications/:id/reminders/mark-taken
   */
  async markMedicationAsTaken(
    ...
    // Permission check (client-side mirror of server validation)
    ...
    await api.patch(
      `/api/health/medications/${medicationId}/reminders/mark-taken`,
      { caregiverId: caregiverId !== memberId ? caregiverId : undefined }
    );
  }
