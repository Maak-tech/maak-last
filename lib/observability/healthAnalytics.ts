...
class HealthAnalyticsService {
  ...
  async getPersonalizedBaseline(
    ...
    // userBaselineService handles baseline storage via a different schema.
    // No per-vitalType REST endpoint is available; return null.
    return null;
  }
  ...
  async updateBaseline(
    ...
    try {
      ...
      // Skip Firestore persistence; baseline is computed in memory only.
      logger.info(
        "Updated patient baseline (in-memory)",
  ...
  private async calculateScoreTrend(
    ...
    // No REST endpoint for historical health scores; return stable.
    return "stable";
  }
